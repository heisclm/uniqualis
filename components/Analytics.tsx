            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={demographicData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <RechartsTooltip 
                  ...
                  contentStyle={{ borderRadius: '16px', border: '1px solid #e2e8f0', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)' }}
                />
                {/* Bars are generated dynamically from the API response keys —
                    only terms / levels with real data are rendered. */}
                {demographicData.length > 0 &&
                  Object.keys(demographicData[0])
                    .filter(key => key !== "term")
                    .map((key, idx) => {
                      // Canonical colour palette for the four standard student levels
                      ...
                      const fallback = ["#a78bfa", "#34d399", "#f59e0b", "#f87171", "#2dd4bf"];
                      const fill  = levelColours[key] ?? fallback[idx % fallback.length];
